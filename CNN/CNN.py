...
class CNN(object):
    ...
    def __init__(self, layerSizes,weight_range:None, filter_size, stride=1, padding=1, weight_initialization="He"):
    ...
    def feedforward(self, x, activation_function):
        ...
        height_out = _compute_size(height, filter_height, 1, 2)
        width_out = _compute_size(width, filter_width, 1, 2)
        ...
        for f in self.conv_filters:
            features.append(ReLU_function(ndimage.convolve(x, f, mode='constant', cval=0.0))) 
        
        NK = N // K
        MLP_input = np.zeros((14,14,32))
        for i in range(0,len(features)):


            pooled = features[i][:NK*K, :NK*K].reshape(NK, K, NK, K).max(axis=(1, 3))
           
            MLP_input[:,:,0] = pooled

        
        output,_,_ = self.MLP.feedforward(MLP_input.reshape(14*14*32),softplus_function)
        print(output)
        print(np.argmax(output))
        """
    ...
    def accuracy(self, validation_data, data_size, activation_function):
        ...
        for i in  range(data_size):
            x = validation_data[0][i].reshape(28,28)
